        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-black text-white tracking-tight flex items-center gap-2 drop-shadow-sm">
              Market Intelligence <span className="text-xs tracking-widest font-bold uppercase bg-indigo-500/20 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded-full ml-2">PRO</span>
            </h1>
          </div>
          <div className="flex items-center gap-4">
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-indigo-500/10 text-indigo-400 rounded-full text-sm font-semibold border border-indigo-500/20 shadow-sm relative overflow-hidden group">
              ...
              <span className="relative z-10">Live Data</span>
            </div>
            <button 
      ...
      <main className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 relative">
        ...
        <div className="bg-slate-900/60 backdrop-blur-md px-6 py-4 rounded-2xl border border-white/5 shadow-2xl flex flex-wrap gap-8 items-center text-sm font-medium text-slate-400 transform transition-all hover:border-white/10 relative z-10">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col"><span className="text-[10px] uppercase tracking-wider font-bold">Crop</span> <span className="text-white font-bold text-base leading-none">{query.crop}</span></div>
          </div>
          <div className="w-px h-8 bg-slate-800 hidden sm:block"></div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-indigo-400 border border-white/5 shadow-inner">📦</div>
            <div className="flex flex-col"><span className="text-[10px] uppercase tracking-wider font-bold">Volume</span> <span className="text-white font-bold text-base leading-none">{query.quantity} Qtl</span></div>
          </div>
          <div className="w-px h-8 bg-slate-800 hidden sm:block"></div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center text-indigo-400 border border-white/5 shadow-inner">📍</div>
            <div className="flex flex-col"><span className="text-[10px] uppercase tracking-wider font-bold">Origin</span> <span className="text-white font-bold text-base leading-none">{query.location}</span></div>
          </div>
          <div className="w-px h-8 bg-slate-800 hidden sm:block"></div>
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col"><span className="text-[10px] uppercase tracking-wider font-bold">Date</span> <span className="text-white font-bold text-base leading-none">{query.date}</span></div>
          </div>
        </div>
        ...
        <div>
          ...
          >
            <Info className="w-4 h-4" /> Why this decision?
          </button>
          {showExplanation && (
            <div className="mt-4 p-5 bg-white border border-blue-100 rounded-xl shadow-inner text-gray-700 leading-relaxed text-sm animate-fade-in-down border-l-4 border-l-blue-500">
              Our AI engine analyzed historical patterns, current trends, and regional pricing differences. 
              The moving average highlights a significant trend which indicates that {decision === 'SELL' ? 'prices might drop soon' : decision === 'WAIT' ? 'prices are likely to recover' : `traveling to ${bestMandi.name} covers logistics costs and yields higher profit`}.
            </div>
          )}
        </div>
