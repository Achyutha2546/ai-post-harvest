...
const MARKET_DATA = {
  Paddy: {
    ...
    insight: "MSP-anchored (₹2,000 floor). Tiruvuru pays ₹70–130 premium. Our model is 97% accurate for Paddy."
  },
  Onion: {
    current: 1059,
    trend: 'RISING (+10.03%)',
    ...
    insight: "⚠️ VOLATILE. Prices swing 26% in a week. GradBoost R² is unreliable (-0.43). Monitor daily."
  },
  Groundnut: {
    current: 7454,
    trend: 'RISING (+5.43%)',
    ...
    insight: "Kurnool pays ₹600–₹990 more than Cuddapah. Dip-recovery in progress (R²=0.90). WAIT for peak."
  },
  Cotton: {
    current: 7680,
    trend: 'STABLE (+0.65%)',
    ...
    insight: "MSP-anchored. Tiruvuru (Krishna dist) gives best realisation. 76% of records favor TRAVEL."
  },
  Chilli: {
    current: 20000,
    trend: 'RISING (+5.66%)',
    ...
    insight: "🚀 SPIKE — ₹20,000 is dataset maximum. Up 19.4% in 14 days. WAIT unless storage risk exists."
  }
};
...
app.post('/analyze', async (req, res) => {
  try {
    ...
    const direction = predictedPrice > currentPrice ? "increase" : "drop";
    const action = decision === 'WAIT'
      ? "hold your harvest"
      : (decision === 'TRAVEL'
        ? `transport your stock to ${bestMandi.name}`
        : "sell immediately");

    let explanation = `${crop} prices are ${data.trend.includes('RISING') ? 'showing positive momentum' : 'relatively stable'}. `;
    explanation += `Current price is ₹${currentPrice} and our regression model predicts a ${direction === 'increase' ? 'growth' : 'change'} to ₹${predictedPrice} (${priceChangePct}%). `;
    explanation += `It is recommended to ${action} to maximize your profit. `;

    if (decision === 'TRAVEL') {
      const extra = (bestMandi.price - currentPrice) * qty;
      explanation += `By moving to ${bestMandi.name}, you gain an additional ₹${extra} total. `;
    }
